...
class YoloV3Adapter(Adapter):
    ...
    def process(self, raw, identifiers, frame_meta):
        ...
        #warnings.warn("2-----------,raw_outputs {}".format( raw_outputs))
#        warnings.warn("raw {}  frame_meta {}".format(raw, frame_meta))
        ...
        #warnings.warn("2-----------,len(result) {}, {},{},{}".format(labels, type(labels),len(labels), identifiers))
...
def multiclass_nms_class_agnostic(boxes, nms_scores, img_size, nms_thr, score_thr):
    """Multiclass NMS implemented in Numpy. Class-agnostic version."""
    labels, scores, x_mins, y_mins, x_maxs, y_maxs = [], [], [], [], [], []

    cls_inds = nms_scores.argmax(1)
    cls_scores = nms_scores[np.arange(len(cls_inds)), cls_inds]
    ...
    if keep:
        ...
        labels = list(valid_cls_inds[keep, None][:,0])
        scores = list(valid_scores[keep, None][:,0])
        x_mins = [x / img_size[1] for x in list(valid_boxes[keep][:,0])] 
        y_mins = [x / img_size[0] for x in list(valid_boxes[keep][:,1])]
        x_maxs = [x / img_size[1] for x in list(valid_boxes[keep][:,2])]
        y_maxs = [x / img_size[0] for x in list(valid_boxes[keep][:,3])]
    return labels, scores, x_mins, y_mins, x_maxs, y_maxs

class YoloxAdapter(Adapter):
    """
    Class for converting output of YOLOx models to DetectionPrediction representation
    """
    __provider__ = 'yolox'
    prediction_types = (DetectionPrediction, )
    ...
    def configure(self):
        ...
        self.p6 = self.get_value_from_config('p6')
        self.raw_output = self.get_value_from_config('raw_output')
    ...
    def process(self, raw, identifiers, frame_meta):
        ...
        for identifier, prediction, meta in zip(identifiers, predictions, frame_meta):
            detections = {'labels': [], 'scores': [], 'x_mins': [], 'y_mins': [], 'x_maxs': [], 'y_maxs': []}
            input_shape = list(meta.get('input_shape', {'data': (1, 3, 640, 640)}).values())[0]
            org_h, org_w = meta['image_size'][:2]
            ratio = min(input_shape[3] / org_w, input_shape[2] / org_h)
            nchw_layout = input_shape[1] == 3
            pred = demo_postprocess(prediction, (input_shape[2], input_shape[3]), self.p6)[0]

            boxes = pred[:, :4]
            scores = pred[:, 4, None] * pred[:, 5:]
            ...
            boxes_xyxy /= ratio
            labels, scores, x_mins, y_mins, x_maxs, y_maxs = multiclass_nms_class_agnostic(boxes_xyxy, scores, (input_shape[2], input_shape[3]), nms_thr=0.45, score_thr=0.1)
